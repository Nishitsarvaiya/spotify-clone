...
const UserPopup = () => {
	...
	return (
		<div className={`${userPopupStatus ? 'block' : 'hidden'} absolute top-14 right-5 z-50`}>
			<div className='min-w-[196px]'>
				<ul className={styles.UserPopupList}>
					...
					<li>
						<button className='w-full flex justify-between items-center p-[10px] gap-2 text-left rounded-sm cursor-default hover:bg-white hover:bg-opacity-10' onClick={() => signOut()}>
							<span className='text-[13px] leading-4 text-white tracking-wider overflow-hidden overflow-ellipsis whitespace-nowrap mt-[4px] flex-1'>
								Log out
							</span>
						</button>
